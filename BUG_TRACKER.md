...
This register tracks verified defects and integration gaps discovered while
reviewing generated artifacts. Status describes the defect, not the associated
implementation task's completion checkbox.

## BG-001: Clean map renderer uses placeholder geometry

- **Status:** Open
- **Task:** [Task 5: Render the clean Leipzig map](IMPLEMENTATION_TASKS.md#5-render-the-clean-leipzig-map)
...
- **Status:** Open
- **Task:** [Task 6: Create the 2:1 Globe Texture](IMPLEMENTATION_TASKS.md#6-create-the-21-globe-texture)
...
- **Done when:** The texture has exactly a 2:1 aspect ratio, derives visibly
  from the rendered map, and seam-offset changes rotate its content without
  changing the source-map pixels.

## BG-003: Offline data stages are disconnected from `build`

- **Status:** Open
- **Tasks:** [Task 4: Derive the Municipal Map](IMPLEMENTATION_TASKS.md#4-derive-the-municipal-map), [Task 5: Render the clean Leipzig map](IMPLEMENTATION_TASKS.md#5-render-the-clean-leipzig-map), and [Task 11: Expose the end-to-end CLI](IMPLEMENTATION_TASKS.md#11-expose-the-end-to-end-cli)
